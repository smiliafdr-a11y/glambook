...
export default function ContactPage() {
  ...
  const FAQ = [
    ...
    { q: "Comment rejoindre la liste d'attente ?", r: "Si aucun créneau ne convient, contactez votre prestataire via ce chat pour être ajoutée à la liste d'attente. Vous serez notifiée dès qu'un créneau se libère." },
    { q: "Quels moyens de paiement ?", r: "Le solde restant le jour J peut être réglé en espèces, par virement ou par carte selon les préférences de votre prestataire." },
  ...
  async function init() {
    ...
    setPrestataire(pres)
    loadMessages(cliente.id, cliente.prestataire_id)

    // Realtime
    supabase.channel('messages').on('postgres_changes', {
      event: 'INSERT', schema: 'public', table: 'messages',
      filter: `cliente_id=eq.${cliente.id}`
    }, () => loadMessages(cliente.id, cliente.prestataire_id)).subscribe()
  }

  async function loadMessages(cId: string, pId: string) {
    const { data } = await supabase.from('messages').select('*')
      .eq('cliente_id', cId).eq('prestataire_id', pId).order('created_at')
    setMessages(data || [])
    setTimeout(() => bottomRef.current?.scrollIntoView({behavior:'smooth'}), 100)
  }

  async function sendMessage() {
    if (!newMsg.trim() || !clienteId || !prestataireId) return
    setSending(true)
    ...
    setNewMsg('')
    setSending(false)
    loadMessages(clienteId, prestataireId)
  }
    ...
    <div className="flex flex-col h-screen pb-16">
      ...
      <div className="p-4 bg-white border-b border-gray-100">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full flex items-center justify-center text-white font-bold" style={{background:'#FF80B5'}}>
        ...
        <div className="flex border border-gray-200 rounded-xl overflow-hidden mt-3">
          <button onClick={() => setOnglet('chat')} className="flex-1 py-2 text-sm font-medium"
            style={{background: onglet==='chat' ? '#FF80B5' : 'white', color: onglet==='chat' ? 'white' : '#9ca3af'}}>
            💬 Chat
          </button>
          <button onClick={() => setOnglet('faq')} className="flex-1 py-2 text-sm font-medium"
      ...
      {onglet === 'chat' ? (
        <>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3" style={{background:'#fdf8fb'}}>
            {messages.length === 0 && (
              <div className="text-center py-8 text-gray-400 text-sm">
                Envoyez un message à votre prestataire 💬
              </div>
            )}
            {messages.map(m => (
          ...
          <div className="px-4 py-2 flex gap-2 overflow-x-auto bg-white border-t border-gray-50" style={{scrollbarWidth:'none'}}>
            {["Bonjour 👋", "C'est quoi le tarif ?", "Est-ce disponible ?", "Merci !"].map(r => (
