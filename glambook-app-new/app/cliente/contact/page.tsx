    ...
    <div style={{display:'flex', flexDirection:'column', height:'calc(100vh - 120px)'}}>
      ...
      <div style={{background:'var(--card)'}}>
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full flex items-center justify-center text-white font-bold text-sm flex-shrink-0" style={{background:'var(--accent)'}}>
            {prestataire?.prenom?.[0]}{prestataire?.nom?.[0]}
          </div>
          <div>
            <div className="font-semibold">{prestataire?.prenom} {prestataire?.nom}</div>
      ...
      {onglet === 'chat' ? (
        <>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3" style={{background:'var(--bg2)'}}>
            {messages.length === 0 && (
              <div className="text-center py-8">
                ...
                <div className="text-theme3 text-sm">Envoyez un message à votre prestataire</div>
              </div>
            )}
            {messages.map(m => (
              <div key={m.id} className={`flex ${m.expediteur === 'cliente' ? 'justify-end' : 'justify-start'}`}>
                <div style={{maxWidth:'75%'}}>
                  {m.expediteur === 'prestataire' && (
                    <div className="text-xs font-semibold mb-1" style={{color:'var(--accent)'}}>{prestataire?.prenom}</div>
                  )}
                  <div className="px-3 py-2 rounded-2xl text-sm leading-relaxed"
                    style={{
                      background: m.expediteur === 'cliente' ? 'var(--accent)' : 'white',
          ...
          {/* RÉPONSES RAPIDES */}
          <div style={{background:'var(--card)', scrollbarWidth:'none'}}>
            ...
            {FAQ.map((f, i) => (
              <div key={i} style={{background:'var(--card)'}}>
                <button onClick={() => setOpenFaq(openFaq===i ? null : i)}
                  ...
                  <span>{f.q}</span>
                  <span className="text-theme3 ml-2 flex-shrink-0">{openFaq===i ? '▲' : '▾'}</span>
                </button>
                {openFaq === i && (
                  <div className="px-4 pb-3 text-sm leading-relaxed border-t border-gray-50">{f.r}</div>
                )}
              </div>
            ))}
          </div>
          <button onClick={() => setOnglet('chat')} className="w-full mt-4 py-3 rounded-xl text-white font-medium text-sm" style={{background:'var(--accent)'}}>
