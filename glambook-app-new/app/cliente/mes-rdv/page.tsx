    ...
    <div className="p-4">
      ...
      <div className="flex border rounded-xl overflow-hidden mb-4">
        <button onClick={() => setOnglet('avenir')} className="flex-1 py-2.5 text-sm font-medium transition-colors"
          style={{background: onglet==='avenir' ? 'var(--accent)' : 'white', color: onglet==='avenir' ? 'white' : '#9ca3af'}}>
          À venir ({rdvsAvenir.length})
        </button>
        <button onClick={() => setOnglet('passes')} className="flex-1 py-2.5 text-sm font-medium transition-colors"
          style={{background: onglet==='passes' ? 'var(--accent)' : 'white', color: onglet==='passes' ? 'white' : '#9ca3af'}}>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {liste.map(rdv => {
            ...
            return (
              <div key={rdv.id} style={{background:'var(--card)'}}>
