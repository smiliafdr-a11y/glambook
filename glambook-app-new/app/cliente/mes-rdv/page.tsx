...
export default function MesRdvPage() {
  ...
  const STATUT: Record<string, {label:string,color:string,bg:string}> = {
    confirme: {label:'Confirmé', color:'var(--accent-d)', bg:'var(--accent-bg)'},
  ...
  return (
    <div className="p-4">
      <h1 className="text-xl font-bold mb-4">Mes rendez-vous</h1>
      ...
      <div className="flex border rounded-xl overflow-hidden mb-4">
        <button onClick={() => setOnglet('avenir')} className="flex-1 py-2.5 text-sm font-medium transition-colors"
          style={{background: onglet==='avenir' ? 'var(--accent)' : 'var(--card)', color: onglet==='avenir' ? 'white' : 'var(--text3)'}}>
          À venir ({rdvsAvenir.length})
        </button>
        <button onClick={() => setOnglet('passes')} className="flex-1 py-2.5 text-sm font-medium transition-colors"
          style={{background: onglet==='passes' ? 'var(--accent)' : 'var(--card)', color: onglet==='passes' ? 'white' : 'var(--text3)'}}>
      ...
      {loading ? (
        ...
        <div className="text-center py-10">
          ...
          <div className="text-theme2 text-sm mb-3">{onglet === 'avenir' ? 'Aucun RDV à venir' : 'Aucun RDV passé'}</div>
          {onglet === 'avenir' && (
            <Link href="/cliente/reserver" className="inline-block text-white text-sm px-4 py-2 rounded-xl" style={{background:'var(--accent)'}}>
              Réserver maintenant
            </Link>
          )}
        </div>
        ...
        <div className="space-y-3">
          {liste.map(rdv => {
            ...
            return (
              <div key={rdv.id} style={{background:'var(--card)', border:'1px solid var(--border)', borderRadius:14, padding:'14px 16px', boxShadow:'var(--shadow)'}}>
                <div className="flex justify-between items-start mb-2">
                  <div>
                    <div className="font-semibold">{(rdv.prestation as any)?.nom}</div>
                ...
                <div className="flex items-center justify-between">
                  <div className="font-bold" style={{color:'var(--accent)'}}>{rdv.prix_total} €</div>
