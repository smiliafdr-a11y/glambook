...
export default function ReserverSansComptePage() {
  ...
  if (confirmed) return (
    <div className="min-h-screen flex items-center justify-center p-4" style={{background:'var(--bg2)'}}>
      <div style={{background:'var(--card)'}}>
        <div className="w-16 h-16 rounded-full flex items-center justify-center text-white text-3xl mx-auto mb-4" style={{background:'var(--accent)'}}>✓</div>
        ...
        <p className="text-xs">📱 SMS de rappel envoyé au {form.telephone}</p>
      </div>
    </div>
  )
  ...
  return (
    <div className="min-h-screen" style={{background:'var(--bg2)'}}>
      <div style={{background:'var(--card)'}}>
        <div className="text-xl font-bold" style={{color:'var(--accent)'}}>GlamBook</div>
        {selectedPrestataire && <div className="text-sm mt-0.5">{selectedPrestataire.prenom} {selectedPrestataire.nom}</div>}
      </div>

      <div className="max-w-lg mx-auto p-4">
        <h1 className="text-xl font-bold mb-1">Réserver un rendez-vous</h1>
        <p className="text-sm mb-5">Sans créer de compte 🙌</p>
          ...
          <div className="space-y-4">
            <div style={{background:'var(--card)'}}>
              ...
              <div className="space-y-3">
                ...
                <div>
                  <label className="block text-xs font-medium mb-1">Nom (optionnel)</label>
                  <input value={form.nom} onChange={e => setForm(f=>({...f,nom:e.target.value}))}
                    className="w-full border rounded-xl px-3 py-2.5 text-sm outline-none"
                    placeholder="Votre nom" />
                </div>
                <div>
                  <label className="block text-xs font-medium mb-1">Téléphone * (rappel SMS)</label>
            ...
            <button onClick={() => { if(form.prenom && form.telephone) setStep(1) }}
              ...
              style={{background:'var(--accent)'}}>Continuer →</button>
          </div>
        )}
        ...
        {step === 1 && (
          <div>
            <button onClick={() => setStep(0)} className="text-sm mb-4" style={{color:'var(--accent)'}}>← Retour</button>
        ...
        {step === 2 && (
          <div>
            <button onClick={() => setStep(1)} className="text-sm mb-4" style={{color:'var(--accent)'}}>← Changer</button>
            <div style={{background:'var(--card)'}}>
                ...
                {Array.from({length: daysInMonth}).map((_,i) => {
                  ...
                  return (
                    <button key={d} onClick={() => { if(dispo){setSelectedDay(d);setSelectedHeure('')}}} disabled={!dispo}
                      ...
                      style={{background: isSelected ? 'var(--accent)' : dispo ? 'var(--accent-bg)' : 'transparent', color: isSelected ? '#fff' : !dispo ? '#ddd' : '#111', border: isSelected ? '2px solid var(--accent)' : dispo ? '1px solid var(--accent-mid)' : '1px solid transparent'}}>
                      {d}
                    </button>
                  )
                })}
            ...
            {selectedDay && selectedHeure && (
              <button onClick={() => setStep(3)} className="w-full py-3 rounded-xl text-white font-semibold" style={{background:'var(--accent)'}}>Continuer →</button>
            )}
          </div>
        )}
        ...
        {step === 3 && (
          <div>
            <button onClick={() => setStep(2)} className="text-sm mb-4" style={{color:'var(--accent)'}}>← Modifier</button>
            <div style={{background:'var(--card)'}}>
              <div className="text-xs font-semibold uppercase tracking-wide mb-3">Récapitulatif</div>
              <div className="space-y-2 text-sm">
                <div className="flex justify-between"><span className="text-theme2">Nom</span><span className="font-medium">{form.prenom} {form.nom}</span></div>
                <div className="flex justify-between"><span className="text-theme2">Téléphone</span><span className="font-medium">{form.telephone}</span></div>
                <div className="flex justify-between"><span className="text-theme2">Prestation</span><span className="font-medium">{selectedPrestation?.nom}</span></div>
                <div className="flex justify-between"><span className="text-theme2">Date</span><span className="font-medium">{selectedDay} {MONTHS[curMonth]} — {selectedHeure}</span></div>
              ...
              <div className="space-y-2">
                <button onClick={() => setModePaiement('especes')} className="w-full p-3 rounded-xl border-2 text-left"
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div><div className="font-medium text-sm">💵 Espèces le jour J</div><div className="text-xs">Total {selectedPrestation?.prix} € sur place</div></div>
                  </div>
                </button>
                {selectedPrestation?.acompte > 0 && (
                  <button onClick={() => setModePaiement('acompte')} className="w-full p-3 rounded-xl border-2 text-left"
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div><div className="font-medium text-sm">💳 Acompte en ligne — {selectedPrestation.acompte} €</div><div className="text-xs">Sécurisez votre créneau maintenant</div></div>
                    </div>
                  </button>
                )}
              </div>
            ...
            <p className="text-xs text-center">SMS de rappel automatique 24h avant 📱</p>
          </div>
        )}
      </div>
    </div>
