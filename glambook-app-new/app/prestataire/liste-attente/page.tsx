    ...
    <div style={{padding:32, maxWidth:760}}>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm mt-0.5">{liste.length} cliente(s) en attente</p>
        </div>
        {liste.length > 0 && (
      ...
      {liste.length === 0 ? (
        ...
        <div className="space-y-3">
          {liste.map((l, i) => (
            <div key={l.id} style={{background:'var(--card)'}}>
              ...
              <div className="flex-1">
                ...
                <div className="flex gap-2 mt-1.5">
                  {l.disponibilites?.matin && <span className="text-xs bg-gray-100 px-2 py-0.5 rounded-full">Matin</span>}
                  {l.disponibilites?.apres_midi && <span className="text-xs bg-gray-100 px-2 py-0.5 rounded-full">Après-midi</span>}
                  {l.disponibilites?.soir && <span className="text-xs bg-gray-100 px-2 py-0.5 rounded-full">Soir</span>}
                </div>
              </div>
