  ...
  return (
    <div style={{padding:32}}>
      <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:24}}>
        <div>
          <h1 className="text-xl font-bold">Mes prestations</h1>
          <p className="text-sm mt-0.5">{prestations.length} prestations configurées</p>
        </div>
      ...
      {prestations.length === 0 ? (
        <div style={{background:'var(--card)'}}>
          ...
          <h3 className="font-semibold mb-2">Aucune prestation</h3>
          <p className="text-sm mb-4">Ajoutez vos prestations pour que les clientes puissent réserver.</p>
        ...
        categories.map(cat => (
          <div key={cat} className="mb-6">
            <h2 className="text-xs font-bold uppercase tracking-wide mb-3">{cat}</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {prestations.filter(p => p.categorie === cat).map(p => (
                <div key={p.id} style={{background:'var(--card)',border:'1px solid var(--border)',borderRadius:12,padding:16,opacity:p.actif?1:0.5}}>
                  <div className="flex items-start justify-between mb-2">
                    <div className="font-semibold text-sm">{p.nom}</div>
                    <div className="flex gap-1">
                      <button onClick={() => openEdit(p)} className="text-xs hover:text-brand-500 px-2 py-0.5 rounded">✏️</button>
                      <button onClick={() => toggleActif(p)} className={`text-xs px-2 py-0.5 rounded ${p.actif ? 'text-green-600 hover:text-red-500' : 'text-theme3 hover:text-green-600'}`}>
                        {p.actif ? '✓' : '○'}
                      </button>
                    </div>
                  </div>
                  {p.description && <p className="text-xs mb-2">{p.description}</p>}
                  <div className="flex items-center justify-between">
                    <div className="text-xs">⏱ {p.duree_minutes < 60 ? `${p.duree_minutes}min` : `${Math.floor(p.duree_minutes / 60)}h${p.duree_minutes % 60 ? (p.duree_minutes % 60) + 'min' : ''}`}</div>
            ...
            <div className="p-5 space-y-3">
              <div>
                ...
                <input value={form.nom} onChange={e => setForm(f => ({ ...f, nom: e.target.value }))}
                  style={{width:'100%', border:'1.5px solid var(--border)', borderRadius:10, padding:'10px 14px', fontSize:13, outline:'none', background:'var(--bg2)', color:'var(--text)'}} placeholder="ex: Pose gel couleur" />
              </div>
              <div>
              ...
              <div>
                ...
                <textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
                  rows={2} style={{width:'100%', border:'1.5px solid var(--border)', borderRadius:10, padding:'10px 14px', fontSize:13, outline:'none', background:'var(--bg2)', color:'var(--text)', resize:'none'}}
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  ...
                  <input type="number" value={form.duree_minutes} onChange={e => setForm(f => ({ ...f, duree_minutes: Number(e.target.value) }))}
                    style={{width:'100%', border:'1.5px solid var(--border)', borderRadius:10, padding:'10px 14px', fontSize:13, outline:'none', background:'var(--bg2)', color:'var(--text)'}} min={15} step={15} />
                </div>
                <div>
                  ...
                  <input type="number" value={form.prix} onChange={e => setForm(f => ({ ...f, prix: Number(e.target.value) }))}
                    style={{width:'100%', border:'1.5px solid var(--border)', borderRadius:10, padding:'10px 14px', fontSize:13, outline:'none', background:'var(--bg2)', color:'var(--text)'}} min={0} />
                </div>
                <div>
                  <label className="block text-xs font-semibold mb-1">Acompte (€)</label>
