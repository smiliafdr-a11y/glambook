      ...
      <div style={{background:'var(--card)'}}>
        <div className="p-4 border-b">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => setShowAddModal(true)} className="text-xs bg-brand-500 text-white px-3 py-1.5 rounded-lg hover:bg-brand-600">+ Ajouter</button>
          </div>
          <input
        ...
        <div className="flex-1 overflow-y-auto">
          {filtered.map(c => (
            <button key={c.id} onClick={() => selectCliente(c)}
              className={`w-full text-left px-4 py-3 border-b border-gray-50 hover:bg-theme2 transition-colors ${selected?.id === c.id ? 'bg-brand-50 border-r-2 border-brand-500' : ''}`}>
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="font-medium text-sm truncate">{c.prenom} {c.nom}</div>
                  <div className="text-xs">{c.nb_prestations} prestations · {c.ca_total} €</div>
                </div>
              </div>
            </button>
          ))}
      ...
      <div className="flex-1 overflow-y-auto p-6">
        {!selected ? (
          ...
          <div className="max-w-2xl">
            ...
            <div style={{background:'var(--card)'}}>
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <h2 className="text-xl font-bold">{selected.prenom} {selected.nom}</h2>
                  ...
                  <div className="flex gap-4 mt-3">
                    <div className="text-center"><div className="font-bold">{selected.nb_prestations}</div><div className="text-xs">Prestations</div></div>
                    <div className="text-center"><div className="font-bold text-brand-500">{selected.ca_total} €</div><div className="text-xs">CA total</div></div>
                    {selected.telephone && <div className="text-sm flex items-center gap-1">📞 {selected.telephone}</div>}
                    {selected.email && <div className="text-sm flex items-center gap-1">✉️ {selected.email}</div>}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div style={{background:'var(--card)'}}>
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-semibold text-sm">📝 Notes privées</h3>
                {!editingNotes ? (
                  <button onClick={() => setEditingNotes(true)} className="text-xs text-brand-500 hover:underline">Modifier</button>
                ) : (
                  <div className="flex gap-2">
                    <button onClick={() => setEditingNotes(false)} className="text-xs hover:text-gray-600">Annuler</button>
            ...
            <div style={{background:'var(--card)'}}>
              <h3 className="font-semibold text-sm mb-4">📋 Historique des prestations</h3>
              {journal.length === 0 ? (
                <div className="text-sm italic">Aucune prestation enregistrée.</div>
              ) : (
                <div className="space-y-3">
                  {journal.map(j => (
                    <div key={j.id} className="flex gap-3 pb-3 border-b border-gray-50 last:border-0">
                      <div className="text-xs min-w-[80px] pt-0.5">
                      ...
                      <div className="flex-1">
                        ...
                        {j.notes_prestataire && (
                          <div className="text-xs rounded p-2 mt-1 border-l-2 border-brand-300 leading-relaxed">
                            {j.notes_prestataire}
                          </div>
                        )}
                        {j.recommandations && (
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input value={newCliente.prenom} onChange={e => setNewCliente(f => ({ ...f, prenom: e.target.value }))}
                    className="w-full border rounded-lg px-3 py-2 text-sm outline-none focus:border-brand-500" placeholder="Amina" />
                </div>
                <div>
                  ...
                  <input value={newCliente.nom} onChange={e => setNewCliente(f => ({ ...f, nom: e.target.value }))}
                    className="w-full border rounded-lg px-3 py-2 text-sm outline-none focus:border-brand-500" placeholder="Benali" />
                </div>
              </div>
              <div>
                <label className="block text-xs font-semibold mb-1">Email</label>
                <input type="email" value={newCliente.email} onChange={e => setNewCliente(f => ({ ...f, email: e.target.value }))}
                  className="w-full border rounded-lg px-3 py-2 text-sm outline-none focus:border-brand-500" placeholder="amina@email.com" />
              </div>
              <div>
                ...
                <input value={newCliente.telephone} onChange={e => setNewCliente(f => ({ ...f, telephone: e.target.value }))}
                  className="w-full border rounded-lg px-3 py-2 text-sm outline-none focus:border-brand-500" placeholder="06 XX XX XX XX" />
              </div>
              <div>
                <label className="block text-xs font-semibold mb-1">Allergies / Contre-indications</label>
                <input value={newCliente.allergies} onChange={e => setNewCliente(f => ({ ...f, allergies: e.target.value }))}
                  className="w-full border rounded-lg px-3 py-2 text-sm outline-none focus:border-brand-500" placeholder="ex: allergie gel UV" />
              </div>
              <div>
                <label className="block text-xs font-semibold mb-1">Note initiale</label>
                <textarea value={newCliente.notes_privees} onChange={e => setNewCliente(f => ({ ...f, notes_privees: e.target.value }))}
                  rows={2} className="w-full border rounded-lg px-3 py-2 text-sm outline-none focus:border-brand-500 resize-none"
