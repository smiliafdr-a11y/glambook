...
const STATUT_COLORS: Record<string, string> = {
  confirme: 'bg-brand-50 text-pink-700',
    ...
    <div className="flex h-full">
      ...
      <div className="flex-1 p-4 md:p-6 overflow-y-auto">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <button onClick={() => changeMonth(-1)} className="w-8 h-8 border rounded-lg flex items-center justify-center hover:bg-theme2">‹</button>
            <h2 className="text-base font-bold min-w-[140px] text-center">{MONTHS[curMonth]} {curYear}</h2>
            <button onClick={() => changeMonth(1)} className="w-8 h-8 border rounded-lg flex items-center justify-center hover:bg-theme2">›</button>
            <button onClick={() => { setCurYear(today.getFullYear()); setCurMonth(today.getMonth()); setSelectedDay(today.getDate()) }}
              className="text-xs px-3 py-1.5 border rounded-lg hover:bg-theme2 ml-1">Aujourd'hui</button>
          </div>
          <button onClick={() => setShowModal(true)} className="text-white text-sm px-4 py-2 rounded-lg font-medium" style={{background:'var(--accent)'}}>
            + Nouveau RDV
          </button>
        </div>

        <div className="grid grid-cols-7 gap-1 mb-1">
          {DAYS.map((d,i) => <div key={d} className={`text-center text-xs font-semibold py-1 ${i >= 5 ? 'text-red-400' : 'text-theme3'}`}>{d}</div>)}
        </div>

        <div className="grid grid-cols-7 gap-1">
          {Array.from({length: startOffset}).map((_,i) => (
            <div key={`p${i}`} className="min-h-[80px] rounded-lg p-1.5 opacity-40">
              <div className="text-xs">{prevDays - startOffset + 1 + i}</div>
            </div>
          ))}
            ...
            return (
              <div key={d} onClick={() => selectDay(d)}
                ...
                style={{
                  ...
                  borderColor: isSelected ? 'var(--accent)' : isToday ? 'var(--accent-mid)' : '#f3f4f6',
                ...
                <div className="flex items-center justify-between mb-1">
                  ...
                  {dayRdvs.length > 0 && <span className="text-xs font-bold" style={{color:'var(--accent)'}}>{dayRdvs.length}</span>}
                </div>
                {isSun ? <div className="text-xs text-gray-300">Fermé</div> : (
                  dayRdvs.slice(0,2).map(r => (
                    <div key={r.id} className={`text-xs px-1 py-0.5 rounded mb-0.5 truncate ${STATUT_COLORS[r.statut] || 'bg-theme2'}`}>
                      {r.heure_debut?.slice(0,5)} {(r.cliente as any)?.prenom}
                    </div>
                  ))
                )}
                {dayRdvs.length > 2 && <div className="text-xs font-medium" style={{color:'var(--accent)'}}>+{dayRdvs.length - 2}</div>}
              </div>
            )
          })}
          {Array.from({length: (7 - (startOffset + daysInMonth) % 7) % 7}).map((_,i) => (
            <div key={`n${i}`} className="min-h-[80px] rounded-lg p-1.5 opacity-40">
      ...
      <div style={{background:'var(--card)'}}>
          ...
          {rdvsJour.length > 0 && (
            <div className="flex gap-2 mt-2">
              <div className="flex-1 rounded-lg p-2 text-center">
                <div className="text-base font-bold" style={{color:'var(--accent)'}}>{rdvsJour.length}</div>
                <div className="text-xs">RDV</div>
              </div>
              <div className="flex-1 rounded-lg p-2 text-center">
                <div className="text-base font-bold text-green-600">{caJour} €</div>
                <div className="text-xs">CA</div>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="flex-1 p-3 space-y-2">
          {rdvsJour.length === 0 ? (
            <div className="text-center py-8">
              ...
              <button onClick={() => setShowModal(true)} className="mt-2 text-xs hover:underline" style={{color:'var(--accent)'}}>+ Ajouter</button>
            </div>
          ) : (
            rdvsJour.map(rdv => (
              <div key={rdv.id} className="border rounded-xl p-3">
                <div className="text-xs font-bold mb-1" style={{color:'var(--accent)'}}>{rdv.heure_debut?.slice(0,5)} – {rdv.heure_fin?.slice(0,5)}</div>
                <div className="font-semibold text-sm">{(rdv.cliente as any)?.prenom} {(rdv.cliente as any)?.nom}</div>
                <div className="text-xs">{(rdv.prestation as any)?.nom} · {rdv.prix_total} €</div>
                {rdv.acompte_paye && <div className="text-xs text-green-600 mt-0.5">Acompte ✓</div>}
                {rdv.notes && <div className="text-xs rounded p-1.5 mt-1.5 border-l-2" style={{borderColor:'var(--accent)'}}>{rdv.notes}</div>}
        ...
        <div className="p-3 border-t">
          <button onClick={() => setShowModal(true)} className="w-full text-white text-sm py-2.5 rounded-lg font-medium" style={{background:'var(--accent)'}}>
            + Ajouter un RDV
          </button>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4"
          ...
          <div style={{background:'var(--card)'}}>
            ...
            <div className="p-5 space-y-4">
              ...
              {/* TOGGLE MODE CLIENTE */}
              <div>
                <label className="block text-xs font-semibold mb-2">Cliente</label>
